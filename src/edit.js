...
export default function Edit({ attributes, setAttributes }) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Announcement Settings', 'apx-gb-announcement')} initialOpen={true}>
					<ToggleControl
						label={__('Show Announcement', 'apx-gb-announcement')}
						checked={show}
						onChange={onToggleShow}
					/>
					{show && <>
						<ToggleControl
							label={__('Automatic Expiration', 'apx-gb-announcement')}
							checked={useExpiryDate}
							onChange={onToggleUseExpiryDate}
						/>
						{useExpiryDate && (
							<DateTimePicker
								label={__('Hide After Date', 'apx-gb-announcement')}
			...
			<div className={isHiddenClass}>
				...
				<RichText
					...
					placeholder={__('Write your announcement here...', 'apx-gb-announcement')}
				/>
			</div>
		</>
	);
